...
- **syncs the catalog to a local mirror** on first add — a paced walk of the API (a few thousand songs take a couple of minutes; the source card shows *"Syncing the FeedForge catalog…"* and songs appear as they arrive), then stays fresh with cheap incremental updates — including **removals**, via FeedForge's deletions feed, so a song taken down from FeedForge disappears from your browse within minutes;
...
- **downloads** a song into your local library the first time you play it. FeedForge hosts nothing itself — each song resolves through the API's tracked download endpoint (so your FeedForge download history and the public counters stay accurate) to an external link — **Google Drive, Dropbox, MediaFire, or a Proton Drive share** — which the client fetches in the background (exactly like the Google Drive / Proton types). A link on a host the plugin doesn't support fails with a clear error instead of downloading a broken package — report it so support can be added.

Your **access key is a secret**: stored locally with the source, sent **only** to feedforge.org, and stripped from every response (like a server access token). Change or clear it any time with the **key** button on the source card. Keys expire after ~180 days — the card starts warning **30 days ahead** (via FeedForge's account-status endpoint), and once a key actually dies it shows **Key required** and you paste a fresh one. The source label defaults to your FeedForge username (a label you typed yourself is never changed), and the client identifies itself to FeedForge honestly as `feedback-remote-library-client/<version>`.
